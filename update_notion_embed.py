    ...
    def search_blocks(block_id):
        ...
        for child in results:
            ...
            if btype == "image":
                ...
                if "study_heatmap" in img_url or "raw.githubusercontent.com" in img_url:
                    return bid

            elif btype == "embed":
                emb_url = child.get("embed", {}).get("url", "")
                if "study_heatmap" in emb_url or "raw.githubusercontent.com" in emb_url:
                    return bid
...
def main():
    ...
    repository = os.getenv("REPOSITORY")    # e.g. "username/study-notion-heatmap"
    ...
    svg_raw_url = get_github_raw_url(repository, ref, "study_heatmap/main.svg")
    ...
    heatmap_display_url = (
        f"https://{username}.github.io/{pages_repo}/study.html?image={svg_raw_url}"
    )
